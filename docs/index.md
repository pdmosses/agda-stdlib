...
!!! note
    ...
    This is an *experimental* website, generated from the same Agda source files
    as the official [standard library website v2.2] using an alternative `Makefile`
    and the theme [Material for MkDocs] with the [Awesome-Nav] plugin.

    The [README] section contains all the `README` modules from the `doc` directory.
    The [Library] section contains the built-in and standard library modules that
    are (transitively) imported by the `README` modules.

    The rest of this file was copied from the README page of the official library
    repository.
